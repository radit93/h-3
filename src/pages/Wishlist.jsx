...
export default function Wishlist() {
  ...
  if (loading) {
    return (
      <div className="p-6">
        <h1 className="text-2xl font-bold mb-6 tracking-wide">Wishlist</h1>
        <div>Memuat...</div>
      </div>
    );
  }
      ...
      {items.length === 0 ? (
        ...
        <div className="space-y-4">
          {items.map((item) => {
            ...
            const variantPrices = item.product.stock_variants?.map(v => Number(v.price)) || [];
            const cheapestPrice = variantPrices.length > 0 ? Math.min(...variantPrices) : 0;
